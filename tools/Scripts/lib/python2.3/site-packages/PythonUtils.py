...
def getGoboVariable(name, filename = 'GoboPath', isList = False) :
	...
	if not isList :
		...
		try :
			...
			for file in files :
				os.environ[name] = os.popen("/bin/zsh -c '. " + file +" 2> /dev/null; echo -n $"+name+"'").read()
				if os.environ[name] :
					break
			return os.environ[name].split()
...
def getCompileOptions() :
	...
	try :
		...
		for file in compileSettingsFiles :
			compileRecipeDirs = os.popen("/bin/zsh -c '. " +file+" 2> /dev/null; echo -n $compileRecipeDirs'").read().split()
	...
	try :
		...
		for file in compileSettingsFiles :
			getRecipeStores = os.popen("/bin/zsh -c '. " + file +" 2> /dev/null; echo -n $getRecipeStores'").read().split()
			if getRecipeStores :
				break
