	...
	if name :
		if not isList :
			try :
				...
				for file in files :
					value = os.popen("/bin/bash -c '. %s 2> /dev/null; echo -n $%s'"%(file,name)).read().strip('\n')
					if value :
						break
			...
			try :
				...
				for file in files :
					os.environ[name] = os.popen("/bin/bash -c '. " + file +" 2> /dev/null; for i in ${"+name+"[@]}; do echo $i; done'").read().strip('\n')
					if os.environ[name] :
						break
				return os.environ[name].split()
	else :
		...
		for file in files :
			ret = os.popen("/bin/bash -c 'cat " + file +" 2> /dev/null'").read().strip('\n')
			if ret :
...
def getCompileOptions() :
	...
	try :
		...
		for file in compileSettingsFiles :
			compileRecipeDirs = os.popen("/bin/bash -c '. " +file+" 2> /dev/null; for i in \"${compileRecipeDirs[@]}\"; do echo $i; done'").read().strip('\n').split('\n')
