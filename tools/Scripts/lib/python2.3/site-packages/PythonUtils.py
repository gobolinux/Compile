...
def getCompileOptions() :
	...
	try :
		...
		for file in compileSettingsFiles :
			compileRecipeDirs = os.popen("/bin/sh -c '. " +file+" 2> /dev/null; echo -n $compileRecipeDirs'").read().split()
	...
	try :
		...
		for file in compileSettingsFiles :
			getRecipeStores = os.popen("/bin/sh -c '. " + file +" 2> /dev/null; echo -n $getRecipeStores'").read().split()
			if getRecipeStores :
				break
